...
class Gtmt extends command_1.Command {
    ...
    async postToSlack(balance, ac, colorStr) {
        ...
        const imgUrl = encodeURI(`https://image-charts.com/chart?cht=lc&chxt=y&chd=a:${balance.join(",")}&chs=999x999&chco=${colorStr}&chdl=${ac}&chl=${label.join("|")}&chm=s,${colorStr},0,-1,13.0|B,${b},0,0,0&chls=5`);
        console.log({ imgUrl });
