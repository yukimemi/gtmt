...
class Gtmt extends command_1.Command {
    ...
    async run() {
        try {
            ...
            const total = _.zipWith(sbi, smbc, ufj, yucho, aeon, oneOpen, worldIndex, (x1, x2, x3, x4, x5, x6, x7) => x1 + x2 + x3 + x4 + x5 + x6 + x7);
            console.log({ total });
            const label = total.map((x, idx) => (idx === total.length - 1 ? x : ""));
            const imgUrl = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=x,y&chxl=0:|${dates.join("|")}&chd=a:${[sbi, smbc, ufj, yucho, aeon, oneOpen, worldIndex]
                .map((x) => x.join(","))
