...
class Gtmt extends command_1.Command {
    ...
    async postToSlack(balance, ac, color) {
        const label = balance.map((x, idx) => idx === balance.length - 1 ? x : "");
        const web = new web_api_1.WebClient(this.slackToken);
        const imgUrl = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${balance.join(",")}&chs=999x999&chco=${color}&chdl=${ac}&chl=${label.join("|")}`);
    ...
    async run() {
        try {
            ...
            await this.postToSlack(worldIndex, "三井住友TAM-世界経済インデックスファンド", "D3D3D3");
