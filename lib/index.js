...
class Gtmt extends command_1.Command {
    ...
    async run() {
        try {
            ...
            const total = _.zipWith(sbi, smbc, ufj, yucho, coincheck, btcbox, bitFlyer, liquid, geo, aeon, oneOpen, worldIndex, (x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12) => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12);
            console.log({ total });
