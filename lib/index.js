...
class Gtmt extends command_1.Command {
    ...
    async run() {
        try {
            ...
            const imgUrlSbi = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${sbi.join(",")}&chs=999x999&chco=1E90FF&chdl=住信SBIネット銀行&chl=${labelSbi.join("|")}`);
            ...
            console.log({ smbc });
            const labelSmbc = smbc.map((x, idx) => idx === smbc.length - 1 ? x : "");
            const imgUrlSmbc = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${smbc.join(",")}&chs=999x999&chco=32CD32&chdl=三井住友銀行&chl=${labelSmbc.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[三井住友銀行] の資産です！",
                attachments: [
            ...
            const imgUrlUfj = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${ufj.join(",")}&chs=999x999&chco=DC143C&chdl=三菱UFJ銀行&chl=${labelUfj.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[三菱UFJ銀行] の資産です！",
                attachments: [
            ...
            console.log({ yucho });
            const labelYucho = yucho.map((x, idx) => idx === yucho.length - 1 ? x : "");
            const imgUrlYucho = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${yucho.join(",")}&chs=999x999&chco=228B22&chdl=ゆうちょ銀行&chl=${labelYucho.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[ゆうちょ銀行] の資産です！",
                attachments: [
            ...
            const labelCoincheck = coincheck.map((x, idx) => idx === coincheck.length - 1 ? x : "");
            const imgUrlCoincheck = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${coincheck.join(",")}&chs=999x999&chco=00FFFF&chdl=coincheck&chl=${labelCoincheck.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
            ...
            console.log({ bitbank });
            const labelBitbank = bitbank.map((x, idx) => idx === bitbank.length - 1 ? x : "");
            const imgUrlBitbank = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${bitbank.join(",")}&chs=999x999&chco=A9A9A9&chdl=bitbank&chl=${labelBitbank.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[bitbank] の資産です！",
                attachments: [
            ...
            console.log({ btcbox });
            const labelBtcbox = btcbox.map((x, idx) => idx === btcbox.length - 1 ? x : "");
            const imgUrlBtcbox = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${btcbox.join(",")}&chs=999x999&chco=FFA500&chdl=BTCBOX&chl=${labelBtcbox.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[BTCBOX] の資産です！",
                attachments: [
            ...
            const labelBitFlyer = bitFlyer.map((x, idx) => idx === bitFlyer.length - 1 ? x : "");
            const imgUrlBitFlyer = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${bitFlyer.join(",")}&chs=999x999&chco=8B008B&chdl=bitFlyer&chl=${labelBitFlyer.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[bitFlyer] の資産です！",
                attachments: [
            ...
            console.log({ liquid });
            const labelLiquid = liquid.map((x, idx) => idx === liquid.length - 1 ? x : "");
            const imgUrlLiquid = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${liquid.join(",")}&chs=999x999&chco=00008B&chdl=Liquid by Quoine&chl=${labelLiquid.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
            ...
            const imgUrlGeo = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${geo.join(",")}&chs=999x999&chco=FFFF00&chdl=ゲオHD&chl=${labelGeo.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[ゲオHD] の資産です！",
                attachments: [
            ...
            console.log({ aeon });
            const labelAeon = aeon.map((x, idx) => idx === aeon.length - 1 ? x : "");
            const imgUrlAeon = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${aeon.join(",")}&chs=999x999&chco=DDA0DD&chdl=イオン&chl=${labelAeon.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
                text: "[イオン] の資産です！",
                attachments: [
            ...
            console.log({ oneOpen });
            const labelOneOpen = oneOpen.map((x, idx) => idx === oneOpen.length - 1 ? x : "");
            const imgUrlOneOpen = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${oneOpen.join(",")}&chs=999x999&chco=2F4F4F&chdl=One-MHAM新興成長株オープン&chl=${labelOneOpen.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
            ...
            const labelWorldIndex = worldIndex.map((x, idx) => idx === worldIndex.length - 1 ? x : "");
            const imgUrlWorldIndex = encodeURI(`https://image-charts.com/chart?cht=bvs&chxt=y&chd=a:${worldIndex.join(",")}&chs=999x999&chco=D3D3D3&chdl=三井住友TAM-世界経済インデックスファンド&chl=${labelWorldIndex.join("|")}`);
            await web.chat.postMessage({
                channel: "#portfolio",
